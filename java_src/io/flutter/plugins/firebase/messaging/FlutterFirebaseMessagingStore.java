...
public class FlutterFirebaseMessagingStore {
    ...
    public void removeFirebaseMessage(String str) {
        ...
        if (!preferencesStringValue.isEmpty()) {
            setPreferencesStringValue(KEY_NOTIFICATION_IDS, preferencesStringValue.replace(str + StringUtils.COMMA, ""));
        }
    }
    ...
    public void storeFirebaseMessage(u0 u0Var) {
        setPreferencesStringValue(u0Var.m(), new JSONObject(FlutterFirebaseMessagingUtils.remoteMessageToMap(u0Var)).toString());
        String str = getPreferencesStringValue(KEY_NOTIFICATION_IDS, "") + u0Var.m() + StringUtils.COMMA;
        ArrayList arrayList = new ArrayList(Arrays.asList(str.split(StringUtils.COMMA)));
        if (arrayList.size() > 100) {
            String str2 = (String) arrayList.get(0);
            getPreferences().edit().remove(str2).apply();
            str = str.replace(str2 + StringUtils.COMMA, "");
        }
