...
public class ProcessGlobalConfigManager extends ChannelDelegateImpl {
    ...
    public void onMethodCall(MethodCall methodCall, MethodChannel.Result result) {
        ...
        if (inAppWebViewFlutterPlugin != null && inAppWebViewFlutterPlugin.activity != null) {
            try {
                ProcessGlobalConfig.a(new ProcessGlobalConfigSettings().parse2((Map) methodCall.argument("settings")).toProcessGlobalConfig(this.plugin.activity));
                result.success(Boolean.TRUE);
                return;
            } catch (Exception e10) {
                result.error(LOG_TAG, "", e10);
                return;
            }
        }
